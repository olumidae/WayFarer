...
const Trip = {
  ...
  async createTrip(req, res) {
    const { error } = authenticateTrip(req.body);
    if (error) return res.status(400).json({ status: 400, error: error.details[0].message });

    const createTrip = `INSERT INTO 
    trip (id, bus_id, origin, destination, trip_date, fare)
    VALUES($1, $2, $3, $4, $5, $6)
    RETURNING *`;
    ...
    if (rows[0].is_admin === false) {
      return res.status(400).json({ status: 'error', error: 'Only admin can create trips' });
    }
